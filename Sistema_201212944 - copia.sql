...
Constraint fk_Asigna_Red_e foreign  Key (empresa) references Empresa(empresa)
) ENGINE=InnoDB;
...
constraint fk_usuario_emp foreign key (empresa) references Empresa(empresa)
)ENGINE=InnoDB;
...
Constraint fk_articulo_empresa Foreign Key (empresa) references Empresa(empresa)
)ENGINE=InnoDB;
...
Constraint fk_asigna_cat Foreign key (categoria) references categoria(categoria),
Constraint fk_asigna_art Foreign key (articulo) references articulo(articulo)
...
constraint fk_sesion_user Foreign Key (usuario) references Usuario(usuario)
)ENGINE=InnoDB;
...
BEGIN
	...
	if not exists((select red from red where nombre=nom)) then
		...
		select 'Error, ya existe una red con ese nombre';
    end if;
END; $
...
Delimiter $
Create procedure Crear_Empresa (in nom varchar(50), in vis varchar(1000), in mis varchar(1000), in mens varchar(1000), in slog varchar(1000), in History varchar(1000), in des varchar(100), in log varchar(50),in p1 varchar(50),in p2 varchar(50),in p3 varchar(50))
BEGIN

	
	if not exists((select nombre from empresa where nombre=nom)) then
		insert into Empresa(nombre,mision, vision, mensaje,historia, slogan, descripcion,logo,portada1,portada2,portada3) values (nom,mis,vis,mens,history,slog,des,log,p1,p2,p3);
	else
...
IF Exists (Select categoria from categoria where categoria= cat) then
	if not exists (select categoria from asigna_articulo where categoria= cat) then
		DELETE FROM `textiles`.`categoria`
		WHERE categoria=cat;
		Select '1' as resultado;
	else 
		select '3' as resultado;
...
Delimiter $
Create procedure Get_8_Categorias()
Begin
select categoria, nombre_categoria, imagen from categoria order by nombre_categoria limit 8;
End $
...
Delimiter $
Create procedure Modificar_Empresa(in vis varchar(1000),in mis varchar(1000),in slo varchar(1000),in hist varchar(1000),in msg varchar(1000),in des varchar(1000))
Begin
UPDATE `textiles`.`empresa`
SET
`Mision` = mis,
